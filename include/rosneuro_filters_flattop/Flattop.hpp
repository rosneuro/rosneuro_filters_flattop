...
namespace rosneuro {
    ...
    bool Flattop<T>::create_window(int nsamples) {
        ...
        for(auto i = 0; i<this->nsamples_; i++) {
            this->window_(i) = this->A0 - this->A1 * cos((2.0 * M_PI * i) / (this->nsamples_ - 1)) +
                               this->A2 * cos((4.0 * M_PI * i) / (this->nsamples_ - 1)) -
                               this->A3 * cos((6.0 * M_PI * i) / (this->nsamples_ - 1)) +
                               this->A4 * cos((8.0 * M_PI * i) / (this->nsamples_ - 1));
        }
